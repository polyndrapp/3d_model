...
faces_for_fill = [
    {'indices': [0, 1, 2], 'color': [1.0, 0.75, 0.8]},
    {'indices': [0, 2, 3], 'color': [1.0, 0.75, 0.8]},
    {'indices': [1, 2, 4], 'color': [0.9, 0.3, 0.4]},
    {'indices': [2, 3, 4], 'color': [0.8, 0.2, 0.3]},
]
...
def draw_line_zbuffered(p0, p1, z_buffer, frame_buffer, color=[0, 0, 0], thickness=2):
    """
    Рисует линию от p0 до p1, проверяя Z-буфер.
    """
    x0, y0, z0 = p0
    x1, y1, z1 = p1
...
def render_scene(projected_verts, faces, outlines, width, height):
    ...
    frame_buffer = np.ones((height, width, 3))  # Белый фон
